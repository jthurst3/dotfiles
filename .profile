...
function sourcepaths() {
    ...
    while read -r line; do
        ...
        if $parsing_variable; then
            current_variable=$line
            current_value=""
            parsing_variable=false
        elif [[ -z $line ]]; then
            current_value=$(echo $current_value | sed 's/^ //' | tr " " ":")
            export $current_variable=$current_value
            parsing_variable=true
        else

            current_value="$current_value $(eval echo $line)"
        fi
    done < ~/.paths
}
sourcepaths
