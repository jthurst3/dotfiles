...
export ZSH=/u/jthurst3/.oh-my-zsh
...
# Prolog 3+4
function moveon() {
    username=$(pwd | awk -F'/' '{ print $6 }')
    cd /localdisk/hassler/Prolog/2015
    cd $(ls -d */ | grep $username -A1 | tail -1)
}
function pprepare() {
    # copy the grading script to their directory and open it up with Vim
    cp /localdisk/hassler/Prolog/{shelltest.sh,test_all.pl} .
    vim *.pl
    ./shelltest.sh
}
